...
namespace CoreXT.System.Platform {
    namespace(() => CoreXT.System.Platform);
    // ========================================================================================================================================
    export interface IEvents { [index: string]: Events.IEventDispatcher<IGraphNode, (ev: Event) => any>; }

    /** A graph item represents a single node on the application graph. */
    export class GraphNode extends FactoryBase(PropertyEventBase) {
        /**
            ...
            */
        static 'new'(parent: IGraphNode = null): IGraphNode { return null; }

        static init(o: IGraphNode, isnew: boolean, parent: IGraphNode = null) {
            ...
            var i: number, n: number, p: IStaticProperty, type = (<any>o).constructor, sp = type.__staticProperties;
        ...
        */
        static registerProperty(factory: IFactory, name: string, isVisual: boolean = false,
            ...
            function exception(_msg: string) {
                return Exception.from("GraphNode.registerProperty(" + getTypeName(factory) + ", " + name + ", " + isVisual + "): " + _msg, this);
            }

            if (!factory) throw exception("A class type [function] is required for property registration.");
            ...
            var currentSP: IArray<IStaticProperty> = factory['__staticProperties'];

            if (currentSP === void 0) {
                factory['__staticProperties'] = currentSP = Array.new<IStaticProperty>();
                (<any>currentSP).parent = null;
                (<any>currentSP).owner = factory;
            }
            else if ((<any>currentSP).owner !== factory) {
                ...
                var typeSpecificSP: IArray = Array.new();
                (<any>typeSpecificSP).parent = currentSP;
                (<any>typeSpecificSP).owner = factory;
        ...
        static accessor = (registeredProperty: IStaticProperty): any => {
            return function (value: any) {
                if (arguments.length > 0)
                    return (<IGraphNode>this).setValue(registeredProperty, value);
                return (<IGraphNode>this).getValue(registeredProperty);
            };
        };
    ...
    export namespace GraphNode {
        export class $__type extends FactoryType(PropertyEventBase) {
            ...
            __parent: IGraphNode = null;
            __children: IArray<IGraphNode> = Array.new<IGraphNode>();
            ...
            */
            ...
            setValue(index: any, value: any, triggerChangeEvents: boolean = true): this {
                ...
                if (typeof value === 'object' && value instanceof Property.$__type)
                    value = (<IProperty>value).getValue(); // (value is a property instance, so get its value)

                var property = this.__properties[name];
                if (property === void 0)
                    this.__properties[name] = property = Property.new(this, null, value); // (property doesn't exist yet, so create it [note: no static info exists])
                else
            ...
            */
            ...
            getProperty(index: any): IProperty {
                var name = index.name || index;
            ...
            __DoOnAnyPropertyChanged(property: IProperty) {
            ...
            */
            ...
            on(eventName: string, handler: (ev: Event) => any) {
                ...
                if (existingEvent === void 0)
                    this.__events[eventName] = existingEvent = Events.EventDispatcher.new<IGraphNode, (ev: Event) => any>(this, eventName);
            ...
            removeAllChildren(): IGraphNode[] {
                var items: IGraphNode[] = this.__children;
            ...
            getItem(id: string): IGraphNode { // TODO: Create a hash list for the IDs instead.
            ...
            */
            ...
            private static [constructor](factory: typeof GraphNode) {
    ...
    export interface IGraphNode extends GraphNode.$__type { }
