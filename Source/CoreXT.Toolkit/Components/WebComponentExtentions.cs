        ...
        public static T SetAttributes<T>(this T comp, object attributes) where T : IWebComponent
        {
            if (attributes != null)
            {
                foreach (var prop in attributes.GetType().GetProperties(BindingFlags.Instance | BindingFlags.Public))
                    comp.Attributes[WebComponent.ObjectMemberNameToAttributeName(prop.Name)] = prop.GetValue(attributes).ND();

                foreach (var field in attributes.GetType().GetFields(BindingFlags.Instance | BindingFlags.Public))
                    comp.Attributes[WebComponent.ObjectMemberNameToAttributeName(field.Name)] = field.GetValue(attributes).ND();
            }
            return comp;
        }
