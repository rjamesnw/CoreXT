...
namespace CoreXT.MVC.Components
{
    ...
    public abstract class WebComponent : TagComponent, IWebComponent
    {
        ...
        public const string ComponentPrefix = "xt-";
        ...
        public static string JoinURLPathNames(IEnumerable<string> names)
            => string.Join("/", names.Where(s => !string.IsNullOrWhiteSpace(s)).Select(s => s.Trim()));
